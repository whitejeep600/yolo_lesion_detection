...
def metadata_to_label(meta: pd.Series) -> YoloFormatLabel:
    ...
    width = x_max - x_min
    height = y_max - y_min
    return YoloFormatLabel(x_center, y_center, width, height, meta["Coarse_lesion_type"])
...
def get_labels_and_splits_from_meta(
    ...
    img_id_to_labels = {img_id: [] for img_id in img_ids}
    img_id_to_split = {}
...
def write_labels_in_yolo_format(
    labels: list[YoloFormatLabel], target_path: Path, preserve_type: bool
) -> None:
    label_text_representations = [label.to_text(preserve_type) for label in labels]
...
def main(raw_images_dir: Path, metadata_path: Path, target_path: Path) -> None:
    target_path.mkdir(parents=True, exist_ok=True)
    split_dirs = {split: target_path / split for split in SPLITS}
    for split_dir in split_dirs.values():
        split_dir.mkdir(parents=True, exist_ok=True)
    image_dirs = {split: split_dirs[split] / IMAGES for split in SPLITS}
    label_dirs = {split: split_dirs[split] / LABELS for split in SPLITS}
    ...
    for batch_dir in tqdm(raw_images_dir.iterdir(), total=n_batches):
        ...
        for series_subdir in batch_subdir.iterdir():
            ...
            for slice_file in series_subdir.iterdir():
                ...
                write_labels_in_yolo_format(img_labels, target_labels_path, untype_or_not)
...
if __name__ == "__main__":
    ...
    main(raw_images_dir, metadata_path, target_path)
